...
** Requirements
*** All
=xorg-server xorg-xrandr xorg-xinit xorg-setxkbmap mlocate atool unzip mono pulseaudio pavucontrol icecat/firefox the_silver_searcher imagemagick ttf-inconsolata ttf-opensans ttf-dejavu aspell aspell-en aspell-sv mpd mpc=
*** All optional
...
#+BEGIN_SRC emacs-lisp
  (defvar my/gui-browser
    (if (my/run-and-is-external-package-installed "icecat")
        "icecat"
      "firefox"))
...
** Volume keys
#+BEGIN_SRC emacs-lisp
  (defvar my/audio-sink (substring  (shell-command-to-string "pacmd list-sinks | grep \"index\"") (string-match "[0-9]" (shell-command-to-string "pacmd list-sinks | grep \"index\"")) -1))
