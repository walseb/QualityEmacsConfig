...
#+BEGIN_SRC emacs-lisp
  ...
  (define-key evil-insert-state-map (kbd "C-u") 'my/evil-scroll-up)
  (define-key evil-normal-state-map (kbd "C-u") 'my/evil-scroll-up)

  (define-key evil-insert-state-map (kbd "C-d") 'my/evil-scroll-down)
...
#+BEGIN_SRC emacs-lisp
  (defhydra my/window-hydra (:color red ;;amaranth
                                    ;; Make hydra work with exwm
                                    :pre (progn
                                           (set-cursor-color "#018220")
    ...
    ;; Move focus
    ("p" evil-window-up "Focus up")
    ("n" evil-window-down "Focus down")
    ("l" evil-window-right "Focus right")
    ("h" evil-window-left "Focus left")

    ;; Move focus to edge window
    ("|" evil-window-mru "Frame border window")

    ;; Move window
    ("P" evil-move-very-top "Move up")
    ("N" evil-move-very-bottom "Move down")
    ("L" evil-move-far-right "Move right")
    ("H" evil-move-far-left "Move left")
    ...
    ;; Resize window
    ("C-p" (evil-window-increase-height 10) "Resize up")
    ("C-n" (evil-window-decrease-height 10) "Resize down")
    ("C-l" (evil-window-decrease-width 10) "Resize right")
    ("C-h" (evil-window-increase-width 10) "Resize left")

    ;; Resize full
    ("M-p" evil-window-set-height "Resize full height")
    ("M-n" evil-window-set-height "Resize full height")
    ("M-l" evil-window-set-width "Resize full width")
    ("M-h" evil-window-set-width "Resize full width")

    ;; Split
    ("v" split-window-horizontally "Split horizontally")
    ("j" split-window-vertically "Split vertically")

    ;; Close window
    ("s" delete-window "Delete")

    ;; Focus on window
    ("d" delete-other-windows "Focus")

    ;; Buffer management
    ("e" counsel-find-file "Find file")
    ("a" ivy-switch-buffer "Switch buffer")
    ("k" (kill-buffer(current-buffer)) "Kill buffer")

    ;; Move around in buffer
    ("C-u" evil-scroll-up "Resize right")
    ("C-d" evil-scroll-down "Resize left")
...
** Keys
#+BEGIN_SRC emacs-lisp
  (evil-define-key 'normal dired-mode-map (kbd "t") 'my/leader-map)
...
#+BEGIN_SRC emacs-lisp
  ...
  (evil-define-key 'normal java-mode-map (kbd "t a s") 'start-eclimd)
  (evil-define-key 'normal java-mode-map (kbd "t a S") 'stop-eclimd)

  (evil-define-key 'normal java-mode-map (kbd "t a g t") 'eclim-java-find-type)
  (evil-define-key 'normal java-mode-map (kbd "t a g d") 'eclim-java-find-declaration)
  (evil-define-key 'normal java-mode-map (kbd "t a g r") 'eclim-java-find-references)
  (evil-define-key 'normal java-mode-map (kbd "t a l") 'eclim-java-find-generic)

  (evil-define-key 'normal java-mode-map (kbd "t a f") 'eclim-java-format)
#+END_SRC
...
** Rebind save key
#+BEGIN_SRC emacs-lisp
  (general-nmap "t s" (general-simulate-key "C-x C-s"))
#+END_SRC
...
** Keys
#+BEGIN_SRC emacs-lisp
  (evil-define-key 'motion my/keys-mode-map (kbd "C-u") 'my/evil-scroll-up)
  (evil-define-key 'motion my/keys-mode-map (kbd "C-d") 'my/evil-scroll-down)
  (evil-define-key 'motion my/keys-mode-map (kbd "t") 'my/leader-map)

  (define-key my/keys-mode-map (kbd "M-o") 'my/window-hydra/body)
...
#+BEGIN_SRC emacs-lisp
  ...
  (dolist (k '(XF86AudioLowerVolume
               ...
               M-o
               ;; Workspaces
               s-a s-n s-e s-t s-o s-s s-h s-d s-i s-r
  ...
  (exwm-input-set-key (kbd "C-t") 'my/leader-map)

  (exwm-input-set-key (kbd "M-o") 'my/window-hydra/body)
