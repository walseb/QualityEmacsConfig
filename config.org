...
#+BEGIN_SRC emacs-lisp
  (defun my/company-mode ()
    ;; company doc-buffer disables visual line mode by default because of some reason, doc-mode doesn't have a hook, it calls the major mode hooks
    (visual-line-mode 1))

;;  (add-hook 'fundamental-mode-m 'my/company-mode)
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  (defvar my/projectile-mode-line-project-name "")

  (defun my/update-projectile-mode-line-project-name()
    (interactive)
    (setq my/projectile-mode-line-project-name (projectile-project-name)))
#+END_SRC

**** Add hook
#+BEGIN_SRC emacs-lisp
  ;; Update when switching buffer

  (add-hook 'switch-window-finish-hook 'my/update-projectile-mode-line-project-name)

  (defvar my/projectile-mode-line-project-name-update-time-interval 5)

  ;; Update git directory in mode line from time to time
  (run-with-timer 0 my/projectile-mode-line-project-name-update-time-interval
                  (if (not (eq my/projectile-mode-line-project-name "-"))
                      'my/update-projectile-mode-line-project-name))

  ;; Update when switching workspace
  (add-hook 'exwm-workspace-switch-hook 'my/update-projectile-mode-line-project-name)
#+END_SRC
...
*** Display directory and name
#+BEGIN_SRC emacs-lisp
  ;; Print file name + Directory
       ...
       (format-mode-line
        (quote
         (
          ...
          (:eval
           (format "[%s]"
                   my/projectile-mode-line-project-name))

          " | "

          ;; exwm workspace number
;;          (exwm-workspace-current-index (format-mode-line "%l"))
 ;;         "|"
