...
*** Quick-peek
#+BEGIN_SRC emacs-lisp
  (straight-use-package '(quick-peek :type git :host github :repo "walseb/quick-peek"))
  (require 'quick-peek)
...
#+BEGIN_SRC emacs-lisp
  ...
  (defun my/macro-record-stop ()
    ...
    (add-to-list 'my/macro-store `(,(concat (number-to-string my/current-macro-number) "-" my/macro-last-name) . ,last-kbd-macro)))

  (defun my/macro-prompt ()
    (completing-read "Macro: " (mapcar (lambda (entry) (interactive) `(,(car entry))) my/macro-store)))

  (defun my/macro-run (count)
    (interactive "P")
    (let ((macro (my/macro-prompt)))
      (setq last-kbd-macro (cdr (seq-find (lambda (entry) (interactive) (string= (car entry) macro)) my/macro-store))))
    (if count
        (dotimes (i count)
          (call-last-kbd-macro))
      (call-last-kbd-macro))
