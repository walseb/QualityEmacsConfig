...
#+BEGIN_SRC emacs-lisp
  ...
  (define-key evil-motion-state-map (kbd "t") 'my/leader-map)
  (define-key evil-normal-state-map (kbd "t") 'my/leader-map)
  (define-key evil-emacs-state-map (kbd "C-t") 'my/leader-map)
#+END_SRC
...
** Fix leader bind in modes
#+BEGIN_SRC emacs-lisp
  (evil-define-key 'normal compilation-mode-map (kbd "t") 'my/leader-map)
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  ...
  (define-key my/leader-map (kbd "k") 'my/kill-curr-buffer)
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  ...
  (setq aw-keys '(?a ?n ?e ?t ?o ?s ?h ?d ?i ?r))

  (setq aw-dispatch-alist
        '(
          (?g aw-swap-window "Swap Windows")
          ;;(?M aw-move-window "Move Window")

          ;; Switch buffer then jump to it
          ...
          (?V my/split-and-follow-vertically)

          (?m aw-split-window-horz "Split Horz Window")
          (?M my/split-and-follow-horozontally)

          ;; Delete one window
          (?l aw-delete-window "Delete Window")
          (?L delete-window)
...
** Keys
#+BEGIN_SRC emacs-lisp
  (evil-define-key 'normal my/keys-mode-map (kbd "C-u") 'my/evil-scroll-up)
  (evil-define-key 'normal my/keys-mode-map (kbd "C-d") 'my/evil-scroll-down)
  (evil-define-key 'normal my/keys-mode-map (kbd "t") 'my/leader-map)

  (define-key my/keys-mode-map (kbd "M-o") 'ace-window)
...
#+BEGIN_SRC emacs-lisp
  ...
  ;; (define-key my/x-menu-map (kbd "b") 'my/launch-browser)
...
#+BEGIN_SRC emacs-lisp
  ...
  (exwm-input-set-key (kbd "C-o") 'ace-window)
...
#+BEGIN_SRC emacs-lisp
  ...
  (define-key my/git-menu-map (kbd "f") 'counsel-projectile-ag)
  ...
  (define-key my/git-menu-map (kbd "b") 'counsel-projectile-switch-to-buffer)
  (define-key my/git-menu-map (kbd "B") 'projectile-ibuffer)
...
#+BEGIN_SRC emacs-lisp
  ...
  (setq my/mode-line-background "#003300")
  (setq my/mode-line-foreground "#D3D3D3")
...
#+BEGIN_SRC emacs-lisp
  ...
  ;;(add-hook 'write-file-hooks 'my/update-total-lines)
;; before save hook
  (add-hook 'switch-window-finish-hook 'my/update-total-lines)
#+END_SRC
