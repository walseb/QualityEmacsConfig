...
** Fix mode line notification face
#+BEGIN_SRC emacs-lisp
  (defun alert-mode-line-notify (info)
    (copy-face 'mode-line 'alert-saved-mode-line-face)
    (let ((color (cdr (assq (plist-get info :severity)
                            alert-severity-colors))))
      (my/alert-blink-mode-line color)))

  (defun my/alert-blink-mode-line (color)
    ;; Doesn't work if you use color directly??
    (setq my/alert-blink-color color)

    (run-with-timer 0.25 nil (lambda () (interactive)
                                (set-face-attribute 'mode-line nil :foreground my/alert-blink-color :background my/alert-blink-color)))

    (run-with-timer 0.5 nil 'my/alert-mode-line-restore)

    (run-with-timer 0.75 nil (lambda () (interactive)
                                (set-face-attribute 'mode-line nil :foreground my/alert-blink-color :background my/alert-blink-color)))

    (run-with-timer 1 nil 'my/alert-mode-line-restore)

    (run-with-timer 1.25 nil (lambda () (interactive)
                                (set-face-attribute 'mode-line nil :foreground my/alert-blink-color :background my/alert-blink-color)))

    (run-with-timer 1.5 nil 'my/alert-mode-line-restore))

  (defun my/alert-mode-line-restore ()
    (set-face-attribute 'mode-line nil :foreground "white" :background "white"))

  ;; Disable auto revert back face
...
** Write mbsync config
#+BEGIN_SRC emacs-lisp
  (defun my/setup-mbsync ()
    (interactive)
    (if (not (file-directory-p "/home/admin/Maildir/"))
        (progn
          (make-directory my/device/maildir)
          (make-directory (concat my/device/maildir "gmail"))
          (make-directory (concat my/device/maildir "gmail/inbox"))))

    (my/create-file-with-content-if-not-exist (expand-file-name "~/.mbsyncrc")
                                              (concat 
                                               "IMAPAccount gmail
  ...
  Path " (concat my/device/maildir "gmail/") "
  Inbox " (concat my/device/maildir "gmail/inbox/") "
...
#+BEGIN_SRC emacs-lisp
  ...
  (define-key my/c-mode-map (kbd "c") 'recompile)
...
#+BEGIN_SRC emacs-lisp
  (add-hook 'eshell-after-prompt-hook (lambda () (interactive) (if (not (= 1 (line-number-at-pos (point))))
                                                                    (progn
                                                                      (alert "Eshell command done!" :severity 'high)
...
#+BEGIN_SRC emacs-lisp
  (eval-after-load 'exwm
   ...
   ;;; Insert
      (evil-define-key 'insert exwm-firefox-evil-mode-map (kbd "p") 'exwm-firefox-core-up-select)
      (evil-define-key 'insert exwm-firefox-evil-mode-map (kbd "n") 'exwm-firefox-core-down-select)
...
* Mail
** Mu4e
Requires =mu= and =mbsync= (or equivalent) to be installed
#+BEGIN_SRC emacs-lisp
  (straight-use-package '(mu4e :type git :host github :repo "djcb/mu"
                               :files ("mu4e/*.el")))
  ...
  (setq mu4e-maildir my/device/maildir)
  (setq mu4e-attachment-dir "~/Downloads")

  (define-key my/leader-map (kbd "M") 'mu4e)

  (defvar my/sync-mail-hook nil)
  (defun my/sync-mail ()
    (interactive)
    ;; Sync mailbox, index mails, signal done
    (async-shell-command "mbsync -a; mu index; echo 'DONE!'")
    ;; Give it 5 seconds to complete, then update modeline, etc
    (run-with-timer 5 nil (lambda () (interactive) (run-hooks 'my/sync-mail-hook))))

  (run-with-timer 60 300 'my/sync-mail)
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  ...
  (add-hook 'my/sync-mail-hook #'mu4e-alert-update-mail-count-modeline)
#+END_SRC

**** Set mode line format
#+BEGIN_SRC emacs-lisp
  (defun my/mu4e-alert-modeline-formatter (mail-count)
    (if (not (zerop mail-count))
        (concat " | Mail [" (number-to-string mail-count) "]")))

  (setq mu4e-alert-modeline-formatter #'my/mu4e-alert-modeline-formatter)
#+END_SRC

*** Faces
#+BEGIN_SRC emacs-lisp
  (add-hook 'mu4e-headers-mode-hook '(lambda () (interactive) (my/toggle-mono-font t)))
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  ...
  ;;; Spray
    ...
    (set-face-attribute 'mu4e-attach-number-face nil :foreground my/foreground-color :background my/background-color)
    (set-face-attribute 'mu4e-cited-1-face nil :foreground my/foreground-color :background my/background-color)
    (set-face-attribute 'mu4e-cited-2-face nil :foreground my/foreground-color :background my/background-color)
    (set-face-attribute 'mu4e-cited-3-face nil :foreground my/foreground-color :background my/background-color)
    (set-face-attribute 'mu4e-cited-4-face nil :foreground my/foreground-color :background my/background-color)
    (set-face-attribute 'mu4e-cited-5-face nil :foreground my/foreground-color :background my/background-color)
    (set-face-attribute 'mu4e-cited-6-face nil :foreground my/foreground-color :background my/background-color)
    (set-face-attribute 'mu4e-cited-7-face nil :foreground my/foreground-color :background my/background-color)
    (set-face-attribute 'mu4e-compose-header-face nil :foreground my/foreground-color :background my/background-color)
    (set-face-attribute 'mu4e-compose-separator-face nil :foreground my/foreground-color :background my/background-color)
    (set-face-attribute 'mu4e-contact-face nil :foreground my/foreground-color :background my/background-color)
    (set-face-attribute 'mu4e-context-face nil :foreground my/foreground-color :background my/background-color)
    (set-face-attribute 'mu4e-conversation-header nil :foreground my/foreground-color :background my/background-color-2)
    (set-face-attribute 'mu4e-conversation-sender-1 nil :foreground my/foreground-color :background my/background-color)
    (set-face-attribute 'mu4e-conversation-sender-2 nil :foreground my/foreground-color :background my/background-color)
    (set-face-attribute 'mu4e-conversation-sender-3 nil :foreground my/foreground-color :background my/background-color)
    (set-face-attribute 'mu4e-conversation-sender-4 nil :foreground my/foreground-color :background my/background-color)
    (set-face-attribute 'mu4e-conversation-sender-5 nil :foreground my/foreground-color :background my/background-color)
    (set-face-attribute 'mu4e-conversation-sender-6 nil :foreground my/foreground-color :background my/background-color)
    (set-face-attribute 'mu4e-conversation-sender-7 nil :foreground my/foreground-color :background my/background-color)
    (set-face-attribute 'mu4e-conversation-sender-8 nil :foreground my/foreground-color :background my/background-color)
    (set-face-attribute 'mu4e-conversation-sender-me nil :foreground my/foreground-color :background my/background-color)
    (set-face-attribute 'mu4e-conversation-unread nil :foreground my/foreground-color :background my/background-color)
    (set-face-attribute 'mu4e-draft-face nil :foreground my/foreground-color :background my/background-color)
    (set-face-attribute 'mu4e-flagged-face nil :foreground my/foreground-color :background my/background-color)
    (set-face-attribute 'mu4e-footer-face nil :foreground my/foreground-color :background my/background-color)
    (set-face-attribute 'mu4e-forwarded-face nil :foreground my/foreground-color :background my/background-color)
    (set-face-attribute 'mu4e-header-face nil :foreground my/foreground-color :background my/background-color)
    (set-face-attribute 'mu4e-header-highlight-face nil :foreground my/foreground-color :background my/background-color)
    (set-face-attribute 'mu4e-header-key-face nil :foreground my/foreground-color :background my/background-color)
    (set-face-attribute 'mu4e-header-marks-face nil :foreground my/foreground-color :background my/background-color)
    (set-face-attribute 'mu4e-header-title-face nil :foreground my/foreground-color :background my/background-color)
    (set-face-attribute 'mu4e-header-value-face nil :foreground my/foreground-color :background my/background-color)
    (set-face-attribute 'mu4e-highlight-face nil :foreground my/foreground-color :background my/background-color)
    (set-face-attribute 'mu4e-link-face nil :foreground my/foreground-color :background my/background-color)
    (set-face-attribute 'mu4e-modeline-face nil :foreground my/foreground-color :background my/background-color)
    (set-face-attribute 'mu4e-moved-face nil :foreground my/foreground-color :background my/background-color)
    (set-face-attribute 'mu4e-ok-face nil :foreground my/foreground-color :background my/background-color)
    (set-face-attribute 'mu4e-region-code nil :foreground my/foreground-color :background my/background-color)
    (set-face-attribute 'mu4e-replied-face nil :foreground my/foreground-color :background my/background-color)
    (set-face-attribute 'mu4e-special-header-value-face nil :foreground my/foreground-color :background my/background-color)
    (set-face-attribute 'mu4e-system-face nil :foreground my/foreground-color :background my/background-color :slant 'italic)
    (set-face-attribute 'mu4e-title-face nil :foreground my/foreground-color :background my/background-color)
    (set-face-attribute 'mu4e-trashed-face nil :foreground my/foreground-color :background my/background-color :strike-through t)
    (set-face-attribute 'mu4e-unread-face nil :foreground my/foreground-color :background my/background-color)
    (set-face-attribute 'mu4e-url-number-face nil :foreground my/foreground-color :background my/background-color)
    (set-face-attribute 'mu4e-view-body-face nil :foreground my/foreground-color :background my/background-color)
    (set-face-attribute 'mu4e-warning-face nil :foreground "red" :background my/background-color)
...
#+BEGIN_SRC emacs-lisp
  (add-hook 'after-init-hook (lambda () (interactive)
                                (if (my/has-config-error)
                                    (my/mode-line-error-list-add "CONFIG ERROR!"))))
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  ...
  (defvar my/eye-timer-mode-line-error "EYE TIMER!")
  ;; In seconds
  (defvar my/eye-timer-break-every (* 21 60))

  (defun my/eye-timer-reset ()
    (interactive)
    (setq my/eye-timer-que 0)
    (setq my/eye-timer-que-formated (my/eye-timer-get-formated-time))
    (my/mode-line-error-list-remove my/eye-timer-mode-line-error))
  ...
  (defun my/eye-timer-add ()
    ...
    (my/mode-line-error-list-add my/eye-timer-mode-line-error))
...
**** Keys
#+BEGIN_SRC emacs-lisp
  (define-key my/system-commands-map (kbd "e") 'my/eye-timer-reset)
#+END_SRC
                   ...
                   (format-mode-line
                    (quote
                     (
                      ...
                      (:eval (if my/mode-line-error-list
                                 (concat (propertize my/mode-line-error-list-formatted 'face '(:background "purple")) " | ")))
