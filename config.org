...
#+BEGIN_SRC emacs-lisp
  ...
  (defun pretty-outlines--propertize-bullet (level bullet)
    ...
    (with-face bullet
               (pcase level
                 (0 '(:inherit outline-1 :underline nil)) ;;:height my/org-level-1-height))
                 (1 '(:inherit outline-2 :underline nil)) ;;:height my/org-level-2-height))
                 (2 '(:inherit outline-3 :underline nil)) ;;:height my/org-level-3-height))
                 (3 '(:inherit outline-4 :underline nil)) ;;:height my/org-level-4-height))
                 (4 '(:inherit outline-5 :underline nil)) ;;:height my/org-level-5-height))
                 (5 '(:inherit outline-6 :underline nil)) ;;:height my/org-level-6-height))
                 (6 '(:inherit outline-7 :underline nil)) ;;:height my/org-level-7-height))
                 (7 '(:inherit outline-8 :underline nil)) ;;:height my/org-level-8-height))
                 (_ nil))))
...
#+BEGIN_SRC emacs-lisp
  ...
  (defun my/isearch-current-screen (forward)
    ...
    (let ((narrow-restore
      ...
      ;; FIXME: Currently this resets narrow when done, fix this with save-restriction
      ;;(widen)
  ;;    (if narrow-restore
...
#+BEGIN_SRC emacs-lisp
  ...
  (defun my/auto-jump-to-definition ()
    ...
    (pcase major-mode
      ...
      ('haskell-mode (xref-find-definitions ""))
      (_
       (if lsp-mode
           (lsp-find-definition)
         (dumb-jump-go)))))
...
*** Eldoc
Eldoc support is broken in lsp and only eats up cpu, disable it
#+BEGIN_SRC emacs-lisp
...
** Keys
#+BEGIN_SRC emacs-lisp
  (define-key my/leader-map (kbd "f") 'artist-mode)
...
** Comment to symbols
#+BEGIN_SRC emacs-lisp
  (defvar my/comment-symbol ?|)
  (add-hook 'prog-mode-hook (lambda () (interactive)
                              (setq-local prettify-symbols-alist
                                          (append
                                           prettify-symbols-alist
                                           `((,(string-trim comment-start) . ,my/comment-symbol))))) t)
