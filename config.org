...
#+BEGIN_SRC emacs-lisp
  ...
  (defun my/launch-firefox ()
    (interactive)
    (setq my/firefox-title-name (completing-read "set title " '(
                                                      ("media")
                                                      ("dev")
                                                      )))
    (let* (
           ...
           (adress-raw (completing-read "url " bookmarks))
           (adress (if (cl-member adress-raw bookmarks :test #'string=)
                       adress-raw
                     (concat "https://www.google.com/search?q=" adress-raw))))

     (start-process "firefox name" nil "firefox" "--new-window" adress)
     (run-with-timer 0.5 nil
                     (lambda () (interactive)
                       (exwm-workspace-rename-buffer (concat my/firefox-title-name " - firefox"))))))
...
#+BEGIN_SRC emacs-lisp
  ...
  (define-key my/open-map (kbd "e") 'my/launch-eww)
#+END_SRC
